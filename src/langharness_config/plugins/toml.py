...
from langharness_config.contracts import ConfigProvider
...
class TOMLConfigPlugin:
    ...
    def _ensure_config(self) -> Path:
        ...
        if not path.exists():
            ...
            template = files("langharness_config").joinpath(
                "config/langharness.toml"
            )
