...
2. 固定 SCOPE_ID 集中到 `src/langharmess_plugin/scope_const.py`，作为唯一
   权威来源，消除当前多处重复定义。
...
- 规范常量散落：`ROOT_SCOPE_ID` 在 `langharmess_scope/model.py`；
  `UI/SERVER/AGENT_SCOPE_ID` 与 `agent_instance_scope_id` 在
  `langharmess_core/scopes.py`；而描述符里全是裸字符串
...
`src/langharmess_plugin/scope_const.py`（新文件）：
...
- `ScopeId` 类型与 `ROOT_SCOPE_ID` 的值仍定义在 `langharmess_scope`
...
- 元数据 key `PLUGIN_SCOPE_ID` / `PLUGIN_SCOPE_CHAIN` / `PLUGIN_KEY`
  ...
  各自的本地定义。
- `langharmess_core/scopes.py` 删除，其消费方
...
- 描述符调整：
  - `src/langharmess_core/plugin.py` 中 agent 作用域模板描述符的
...
- 新路由插件 `src/langharmess_api/plugins/routes/scopes.py`
...
- `langharmess_api/plugin.py` 增加 `api-scopes` 描述符
...
- 新命令插件 `src/langharmess_cli/plugins/commands/scope.py`
...
- `langharmess_cli/plugin.py` 注册 `cli-scope` 描述符
...
- 新增 `src/langharmess_plugin/scope_const.py`
- 新增 `src/langharmess_api/plugins/routes/scopes.py`
- 新增 `src/langharmess_cli/plugins/commands/scope.py`
- 修改 `src/langharmess_plugin/plugin_manager.py`（导入 + 种子）
- 修改 `src/langharmess_plugin/scope_policy.py`（导入常量）
- 修改 `src/langharmess_plugin/contracts.py`（scopes() 协议）
- 修改 `src/langharmess_plugin/coordinator.py`（冒号格式、scope_parent、
  恢复语义）
- 删除 `src/langharmess_core/scopes.py`；修改
  `src/langharmess_core/plugin.py`、`src/langharmess_core/plugins/agents/directory.py`
- 修改 `src/langharmess_api/plugin.py`、`src/langharmess_cli/plugin.py`
- 修改/新增对应测试文件
